...
class AuthCore {
  ...
  Future<String?> deleteAccount() async {
    ...
    try {
      // 1. Firestore 하위 컬렉션 삭제 (equipments)
      final equipments =
      ...
      for (final doc in equipments.docs) {
...
class Synchronizer {
  ...
  void _onUserDocumentChanges(
    ...
    if (documentSnapshot.exists && (documentSnapshot.data() != null)) {
      ...
      if (_syncedCustomUser != null) {
        ...
        print("AuthCore: Periodic Timer Started");
  ...
  Future<void> _periodicTick(Timer timer) async {
    if (_syncedCustomUser != null) {
      ...
      final DateTime? lastGaugeDecreaseAt =
          _syncedCustomUser!.lastGaugeDecreaseAt;
      final DateTime now = koreaNow();

      if (now.second % 10 == 0) {
        print("AuthCore: _periodicTick called at $now");
      }
      ...
      if ((nextPeriodic12 == null) || (nextPeriodic24 == null)) {
        shouldTick = true;
        print("AuthCore: shouldTick=true (NextPeriodic is null)");
      } else if (now.isAfter(nextPeriodic12) || now.isAfter(nextPeriodic24)) {
        shouldTick = true;
        print(
          "AuthCore: shouldTick=true (Time passed: $now > $nextPeriodic12 / $nextPeriodic24)",
        );
      }

      if (!shouldTick) {
        final Duration delay = App.instance.reserved.global.gaugeDecreaseDelay;
        final int diffInSeconds =
            lastGaugeDecreaseAt != null
                ? now.difference(lastGaugeDecreaseAt).inSeconds
                : -1;

        // 10초에 한 번씩 로그 출력 (로그 폭주 방지)
        // if (now.second % 10 == 0) {
        //   print(
        //     "Tick Check\n"
        //     "  - Next12: $nextPeriodic12\n"
        //     "  - Next24: $nextPeriodic24\n"
        //     "  - LastGauge: $lastGaugeDecreaseAt\n"
        //     "  - Diff: ${diffInSeconds}s / ${delay.inSeconds}s",
        //   );
        // }

        if (lastGaugeDecreaseAt == null || (diffInSeconds >= delay.inSeconds)) {
          shouldTick = true;
        }
      }
  ...
  Future<void> __periodicTick({required DateTime now}) async {
    print("AuthCore: __periodicTick Start");
    try {
      ...
      await FirebaseFirestore.instance.runTransaction<void>((
        ...
        print("AuthCore: Transaction Start");
        ...
        if (user != null) {
          print("AuthCore: User loaded in transaction");
          ...
          // 3. 리스닝 게이지 주기적 감소 로직 (아이템 03 효과 적용)
          final DateTime? lastGaugeDecreaseAt = user.lastGaugeDecreaseAt;
          final Duration delay =
              App.instance.reserved.global.gaugeDecreaseDelay;

          if (lastGaugeDecreaseAt == null ||
              now.difference(lastGaugeDecreaseAt) >= delay) {
            final Map<String, dynamic> gaugeData = {
              "lastGaugeDecreaseAt": Timestamp.fromDate(now),
            };

            if (!user.inListeningGaugeFreeze) {
              final int decreaseAmount =
                  App.instance.reserved.global.gaugeDecrease;

              // 경과된 시간만큼 누적해서 감소 (백그라운드 처리 대응)
              int decreaseCount = 1;
              if (lastGaugeDecreaseAt != null) {
                final int diffSeconds =
                    now.difference(lastGaugeDecreaseAt).inSeconds;
                final int delaySeconds = delay.inSeconds;
                if (delaySeconds > 0) {
                  decreaseCount = diffSeconds ~/ delaySeconds;
                }
              } else {
                // lastGaugeDecreaseAt이 없으면(최초 실행 등) 바로 감소시키지 않고 기준 시간만 잡을 수도 있으나,
                // 기존 로직(null일 때도 진입)을 존중하여 1회 감소하거나,
                // 혹은 0으로 설정하여 이번엔 감소 안 하고 타이머만 시작하게 할 수 있음.
                // 여기서는 "백그라운드 처리"가 주 목적이므로, 최초 1회는 감소시키지 않고 타이머 시작점으로만 활용 (count = 0)
                decreaseCount = 0;
              }

              if (decreaseCount > 0 && user.listeningGauge > 0) {
                final int totalDecrease = decreaseAmount * decreaseCount;
                gaugeData["listeningGauge"] =
                    (user.listeningGauge - totalDecrease).clamp2();

                print(
                  "--> [Gauge Decrease] $decreaseCount회 누적 감소 (-$totalDecrease)",
                );
              }
            }

            transaction.update(userReference, gaugeData);
          }
        }
      });
    } catch (e, stack) {
      print("❌ AuthCore: __periodicTick Failed: $e");
      print(stack);
    }
  }
