      ...
      await event.map(
        ...
        tabPressed: (event) async {
          if (state.selectedSocketIndex == null) {
            if (state.selectedGearCategory != event.gearCategory) {
              final Pack? mountedPack = state.packs.firstWhereOrNull(
                (element) =>
                    (element.gear.category == event.gearCategory) &&
                    element.mounted,
              );

              emit(
                state.copyWith(
                  selectedGearCategory: event.gearCategory,
                  selectedPack: mountedPack,
                ),
              );
            }
          }
        ...
        onPacksChanges: (event) {
          ...
          if (selectedPack == null) {
            selectedPack = packs.firstWhereOrNull(
              (element) =>
                  (element.gear.category == state.selectedGearCategory) &&
                  element.mounted,
            );
          }
